...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tc-table { width: 100%; border-collapse: collapse; min-width: 520px; }
  .tc-table th {
    padding: 10px 12px;
    text-align: left;
    font-size: 11px;
  ...
  /* ── Column guide ── */
  .tc-col-guide-item { font-size: 11px; color: rgba(255,255,255,0.45); line-height: 1.6; }
  ...
  .tc-filter-row .tc-input { width: 200px; min-width: 0; flex-shrink: 1; }
  ...
  @media (max-width: 640px) {
    ...
    .tc-col-grid { grid-template-columns: 1fr; }
    .tc-filter-row { gap: 8px; }
    .tc-filter-row .tc-input { width: 100%; }
    .tc-filter-row .tc-select { flex: 1 1 calc(50% - 4px); min-width: 0; }
    .tc-actions-row { margin-left: 0; width: 100%; justify-content: flex-end; }
    .tc-input { font-size: 16px !important; } /* prevents iOS auto-zoom */
    .tc-card { padding: 16px; }
  }

  /* ── Very small ── */
  @media (max-width: 380px) {
    .tc-stats-grid { grid-template-columns: 1fr 1fr; gap: 8px; }
    .tc-tab-btn { padding: 10px 12px; font-size: 12px; }
  }
...
export default function CurriculumImport() {
  ...
  const handleSubmit = async () => {
    if (!form.courseId || !form.effectiveYear || !form.curriculumName || !form.file) {
      setMsg({ type: "error", text: "All fields and a file are required." }); return;
    }
  ...
  const pagePad = { padding: "clamp(1rem, 4vw, 2.5rem) clamp(1rem, 4vw, 2.5rem)" };
    ...
    <div style={{ minHeight: "100vh", background: "#060D1A", color: "#F1F5F9", fontFamily: "'DM Sans',sans-serif", ...pagePad }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ marginBottom: 28, animation: "slideUp .5s ease both" }}>
          ...
          <h1 style={{ fontSize: "clamp(1.4rem, 4vw, 1.9rem)", fontWeight: 800, color: "#fff", letterSpacing: "-.02em", fontFamily: "'Sora',sans-serif" }}>Curriculum Management</h1>
        ...
        {/* Course selector */}
        <div className="tc-course-row" style={{ animation: "slideUp .5s ease .05s both" }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.4)", letterSpacing: ".1em", textTransform: "uppercase", fontFamily: "'DM Mono',monospace", flexShrink: 0 }}>Course</span>
        ...
        {/* Tabs */}
        <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: 28, animation: "slideUp .5s ease .1s both", overflowX: "auto" }}>
          ...
          <div style={{ animation: "slideUp .4s ease both" }}>
            {msg.text && (
              <div style={{
                ...
                border: `1px solid ${msg.type === "error" ? "rgba(220,38,38,0.3)" : "rgba(34,197,94,0.3)"}`,
                borderRadius: 10, padding: "10px 16px", marginBottom: 16,
                color: msg.type === "error" ? "#f87171" : "#4ADE80",
                fontSize: 13, display: "flex", gap: 8, alignItems: "flex-start", lineHeight: 1.5,
              }}>
            ...
            <div className="tc-card" style={{ marginBottom: 22 }}>
              ...
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", marginBottom: 20, lineHeight: 1.6 }}>
                Required columns: <code style={{ color: "#86EFAC", background: "rgba(34,197,94,0.1)", padding: "1px 6px", borderRadius: 4, fontSize: 11, wordBreak: "break-all" }}>subject_code, subject_name, units, prerequisite, year_level, semester, subject_type, session_type, has_lab</code>
              </p>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <input type="file" accept=".xlsx,.csv" onChange={e => setForm(f => ({ ...f, file: e.target.files[0] }))}
                  style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", width: "100%" }} />
              </div>

              {/* Column guide */}
              <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.07)", padding: "14px 16px", marginBottom: 22 }}>
              ...
              <button className="tc-btn-primary" onClick={handleSubmit} disabled={saving} style={{ width: "100%", maxWidth: 220 }}>
          ...
          <div style={{ animation: "slideUp .4s ease both" }}>
            ...
            <div className="tc-stats-grid">
              {[
                ...
                <div key={s.label} className="tc-card" style={{ padding: "16px 18px", background: s.accent, borderColor: `${s.color}20` }}>
                  <div style={{ fontSize: "clamp(22px, 5vw, 30px)", fontWeight: 800, color: s.color, fontFamily: "'Sora',sans-serif", letterSpacing: "-.02em" }}>{s.value}</div>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginTop: 4, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".06em", lineHeight: 1.4 }}>{s.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="tc-filter-row">
              <input className="tc-input" placeholder="Search subject…" value={search} onChange={e => setSearch(e.target.value)} />
              ...
              {(search || filterYear !== "ALL" || filterSem !== "ALL" || filterType !== "ALL") && (
                <button className="tc-btn-ghost" onClick={() => { setSearch(""); setFilterYear("ALL"); setFilterSem("ALL"); setFilterType("ALL"); }}>✕ Clear</button>
              )}
              <div className="tc-actions-row">
                <button className="tc-btn-ghost" onClick={handlePrint}>🖨 Print</button>
                <button className="tc-btn-primary" style={{ padding: "9px 16px" }} onClick={handleDownload}>⬇ Download</button>
              </div>
            </div>
                ...
                Object.keys(grouped).sort((a, b) => a - b).map(year => (
                  <div key={year} className="year-block" style={{ marginBottom: 32 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                      <div style={{ background: "linear-gradient(135deg,rgba(34,197,94,0.2),rgba(34,197,94,0.08))", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 10, padding: "5px 16px", fontSize: 12, fontWeight: 800, color: "#4ADE80", fontFamily: "'DM Mono',monospace", letterSpacing: ".06em", textTransform: "uppercase", flexShrink: 0 }}>
                    ...
                    {SEM_ORDER.filter(s => grouped[year][s]).map(sem => (
                      <div key={sem} style={{ marginBottom: 16 }}>
                        ...
                        <div className="tc-card" style={{ padding: 0, overflow: "hidden" }}>
                          <div className="tc-table-wrap">
                            <table className="tc-table">
                              <thead><tr>{["#", "Code", "Subject Name", "Units", "Prereq.", "Type", "Assigned Teacher"].map(h => <th key={h}>{h}</th>)}</tr></thead>
                              <tbody>
                                {grouped[year][sem].map((cs, i) => {
                                  ...
                                  return (
                                    <tr key={cs.id}>
                                      ...
                                      <td style={{ color: "#F1F5F9", fontWeight: 500, minWidth: 160 }}>{cs.subject?.name ?? "—"}</td>
