...
export default function PreferenceReview() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Build pref map from ALL subjects (not just selected course) so teacher vote counts are accurate
  ...
  return (
    <div style={{ padding: "2rem 2.5rem", maxWidth: 1200, margin: "0 auto", fontFamily: "'DM Sans', sans-serif" }}>
      <h1 style={{ fontSize: "1.75rem", fontWeight: 700, color: "#111827", marginBottom: 4 }}>Preference Review & Assignment</h1>
      <p style={{ color: "#6b7280", fontSize: 14, marginBottom: 16 }}>Select a course and teacher, then assign them to a subject.</p>

      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        {courses.length > 0 && (
          <select value={selectedCourseId ?? ""} onChange={e => { setSelectedCourseId(Number(e.target.value)); setSelectedTeacher(null); }}
            style={{ padding: "7px 10px", border: "1.5px solid #1a56db", borderRadius: 8, fontSize: 13, fontWeight: 600, color: "#1a56db" }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 16, alignItems: "start" }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {teachers.length === 0 ? <p style={{ fontSize: 13, color: "#9ca3af" }}>No teachers found.</p>
              : teachers.map(t => {
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {grouped.length === 0 ? (
              <p style={{ fontSize: 13, color: "#9ca3af", padding: 20, textAlign: "center" }}>No preferences for this term yet.</p>
            ) : grouped.map(group => {
                ...
                <div key={subjectId} style={{ border: `1px solid ${isAssigned ? "#bbf7d0" : voted && selectedTeacher ? "#93c5fd" : "#e5e7eb"}`, borderRadius: 10, padding: "10px 14px", background: isAssigned ? "#f0fdf4" : voted && selectedTeacher ? "#eff6ff" : "#f9fafb", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontSize: 11, padding: "1px 8px", borderRadius: 99, fontWeight: 500, background: group.subject?.hasLab ? "#f0fdf4" : "#fef9ee", color: group.subject?.hasLab ? "#15803d" : "#92400e" }}>
                  ...
                  <div style={{ flexShrink: 0 }}>
                    {isAssigned ? (
                      ...
                      <span style={{ fontSize: 12, color: "#9ca3af" }}>{selectedTeacher ? "Did not vote" : "—"}</span>
                    )}
                  </div>
                </div>
              );
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {modalTeacher.preferences.length === 0 ? <p style={{ color: "#9ca3af", fontSize: 13 }}>No preferences submitted.</p>
                : modalTeacher.preferences.map((p, i) => (
                  <div key={i} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#f9fafb" }}>
                    <div>
                    ...
                    <span style={{ fontSize: 11, padding: "2px 10px", borderRadius: 6, fontWeight: 600, background: p.status === "APPROVED" ? "#d1fae5" : p.status === "REJECTED" ? "#fee2e2" : "#fef3c7", color: p.status === "APPROVED" ? "#065f46" : p.status === "REJECTED" ? "#991b1b" : "#92400e" }}>
                      {p.status ?? "PENDING"}
                    </span>
                  </div>
                ))}
