...
const TC_STYLES = `
  ...
  .tc-rm-btn-danger {
    ...
    color:#fca5a5; cursor:pointer; font-family:'DM Sans',sans-serif; transition:all 0.15s; white-space:nowrap;
  }
  ...
  .tc-rm-table { width:100%; border-collapse:collapse; min-width:580px; }
  .tc-rm-table thead tr { background:rgba(255,255,255,0.03); border-bottom:1px solid rgba(255,255,255,0.07); }
  .tc-rm-table th { padding:13px 14px; text-align:left; font-size:10px; font-weight:700; color:rgba(255,255,255,0.28); letter-spacing:0.12em; text-transform:uppercase; font-family:'DM Mono',monospace; white-space:nowrap; }
  .tc-rm-table td { padding:12px 14px; font-size:13px; color:rgba(255,255,255,0.7); border-bottom:1px solid rgba(255,255,255,0.04); font-family:'DM Sans',sans-serif; transition:background 0.15s; }
  ...
  .tc-rm-create-panel { background:rgba(34,197,94,0.04); border:1px solid rgba(34,197,94,0.18); border-radius:16px; padding:clamp(16px,3vw,24px) clamp(16px,3vw,26px); margin-bottom:22px; animation:tcSlideIn 0.3s ease both; backdrop-filter:blur(12px); }

  .tc-rm-search-wrap { position:relative; width:100%; max-width:260px; }
  ...
  /* Stats strip */
  .tc-rm-stats { display:flex; gap:12px; margin-bottom:28px; flex-wrap:wrap; }
  .tc-rm-stat-item { background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.07); border-radius:12px; padding:12px 16px; display:flex; align-items:center; gap:10px; flex:1 1 110px; }

  /* Toolbar */
  .tc-rm-toolbar { display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; gap:10px; flex-wrap:wrap; }
  .tc-rm-toolbar-left { display:flex; gap:10px; flex-wrap:wrap; align-items:center; flex:1; min-width:0; }

  /* Form grids */
  .tc-rm-grid-2 { display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:14px; }
  .tc-rm-grid-cap { display:grid; grid-template-columns:200px 1fr; gap:14px; margin-bottom:18px; align-items:end; }

  @media (max-width:640px) {
    .tc-rm-search-wrap { max-width:100%; }
    .tc-rm-toolbar { flex-direction:column; align-items:stretch; }
    .tc-rm-toolbar-left { flex-direction:column; }
    .tc-rm-toolbar-left .tc-rm-input { width:100%; }
    .tc-rm-toolbar .tc-rm-btn-primary, .tc-rm-toolbar .tc-rm-btn-ghost { width:100%; justify-content:center; }
    .tc-rm-grid-2 { grid-template-columns:1fr; }
    .tc-rm-grid-cap { grid-template-columns:1fr; }
  }
`;
...
export default function ManageRooms() {
  ...
  useEffect(() => {
    api.get("/rooms/campuses").then(r => setCampuses(r.data?.data ?? r.data ?? [])).catch(() => {});
  }, []);
  ...
  const handleCreate = async e => {
    ...
    try {
      await api.post("/rooms", { campusId: Number(form.campusId), name: form.name, roomNumber: form.roomNumber, capacity: Number(form.capacity), roomType: form.roomType });
  ...
  return (
    <div className="tc-rm" style={{ color: "#fff", fontFamily: "'DM Sans',sans-serif", background: "#070f1e", minHeight: "100vh", padding: "clamp(16px,4vw,32px)" }}>
      ...
      <div style={{ marginBottom: 30 }}>
        ...
        <h1 style={{ fontFamily: "'Sora',sans-serif", fontSize: "clamp(22px,5vw,28px)", fontWeight: 800, color: "#fff", letterSpacing: "-0.03em", margin: "0 0 6px" }}>Rooms</h1>
        <p style={{ fontSize: 14, color: "rgba(255,255,255,0.38)", margin: 0 }}>Manage classrooms, labs, and lecture halls across all campuses.</p>
      </div>

      {/* Stats */}
      <div className="tc-rm-stats">
        {[
          ...
          <div key={s.label} className="tc-rm-stat-item">
            <span style={{ fontFamily: "'Sora',sans-serif", fontSize: "clamp(18px,3vw,24px)", fontWeight: 800, color: s.color, letterSpacing: "-0.03em", lineHeight: 1 }}>{s.value}</span>
      ...
      {/* Toolbar */}
      <div className="tc-rm-toolbar">
        <div className="tc-rm-toolbar-left">
          <div className="tc-rm-search-wrap">
          ...
          <select className="tc-rm-input" style={{ width: "clamp(120px,20vw,160px)", colorScheme: "dark" }} value={campusFilter} onChange={e => setCampusFilter(e.target.value)}>
            <option value="">All Campuses</option>
            {campuses.map(c => <option key={c.id} value={c.id}>{c.name ?? c.code}</option>)}
          </select>
          <select className="tc-rm-input" style={{ width: "clamp(120px,18vw,150px)", colorScheme: "dark" }} value={typeFilter} onChange={e => setTypeFilter(e.target.value)}>
            <option value="">All Types</option>
            <option value="LECTURE">Lecture</option>
            <option value="LABORATORY">Laboratory</option>
          </select>
        </div>
        <button className={showForm ? "tc-rm-btn-ghost" : "tc-rm-btn-primary"} onClick={() => { setShowForm(f => !f); setSaveMsg(null); }}>
      ...
      {showForm && (
        <div className="tc-rm-create-panel">
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(34,197,94,0.15)", border: "1px solid rgba(34,197,94,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, flexShrink: 0 }}>🏫</div>
          ...
          <form onSubmit={handleCreate}>
            <div className="tc-rm-grid-2">
              <div>
                <label className="tc-rm-label">Campus</label>
                <select className="tc-rm-input" value={form.campusId} required onChange={e => setForm(f => ({ ...f, campusId: e.target.value }))} style={{ colorScheme: "dark" }}>
              ...
              <div>
                ...
                <select className="tc-rm-input" value={form.roomType} onChange={e => setForm(f => ({ ...f, roomType: e.target.value }))} style={{ colorScheme: "dark" }}>
                  <option value="LECTURE">🎓 Lecture</option>
                  <option value="LABORATORY">🔬 Laboratory</option>
                </select>
              </div>
            ...
            <div className="tc-rm-grid-2">
              <div>
                ...
                <input className="tc-rm-input" value={form.name} required onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. LI Lecture Room 101" />
              </div>
              <div>
                <label className="tc-rm-label">Room Number</label>
                <input className="tc-rm-input" value={form.roomNumber} onChange={e => setForm(f => ({ ...f, roomNumber: e.target.value }))} placeholder="e.g. 101" />
              </div>
            </div>
            <div className="tc-rm-grid-cap">
              <div>
                <label className="tc-rm-label">Capacity</label>
                <input className="tc-rm-input" type="number" min={1} value={form.capacity} required onChange={e => setForm(f => ({ ...f, capacity: e.target.value }))} placeholder="e.g. 45" />
              </div>
              <div />
            </div>
            <button type="submit" className="tc-rm-btn-primary" disabled={saving}>{saving ? "Saving…" : "Create Room"}</button>
          </form>
        </div>
      )}
      ...
      <div className="tc-rm-table-wrap">
        <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 11, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace", letterSpacing: "0.08em", marginLeft: 6 }}>ROOMS — {filtered.length} RECORDS</span>
          </div>
        ...
        <table className="tc-rm-table">
          <thead>
            <tr><th>#</th><th>Room Name</th><th>No.</th><th>Campus</th><th>Type</th><th>Cap.</th><th>Status</th><th style={{ textAlign: "right" }}>Actions</th></tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={8} style={{ textAlign: "center", padding: 48, color: "rgba(255,255,255,0.2)" }}>
              ...
              <tr><td colSpan={8} style={{ textAlign: "center", padding: 56, color: "rgba(255,255,255,0.18)", fontFamily: "'DM Mono',monospace", fontSize: 12, letterSpacing: "0.06em" }}>NO ROOMS FOUND</td></tr>
            ) : filtered.map((r, i) => {
              ...
              return (
                <tr key={r.id}>
                  ...
                  <td>{r.roomNumber ? <span style={{ fontFamily: "'DM Mono',monospace", fontSize: 12, color: "rgba(255,255,255,0.45)", background: "rgba(255,255,255,0.05)", padding: "3px 9px", borderRadius: 6, border: "1px solid rgba(255,255,255,0.08)" }}>{r.roomNumber}</span> : <span style={{ color: "rgba(255,255,255,0.2)" }}>—</span>}</td>
                  <td style={{ color: "rgba(255,255,255,0.55)" }}>{r.campus?.name ?? r.campus?.code ?? "—"}</td>
                  <td>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 7, fontSize: 11, fontWeight: 700, background: isLab ? "rgba(139,92,246,0.1)" : "rgba(59,130,246,0.1)", color: isLab ? "#c4b5fd" : "#93c5fd", border: `1px solid ${isLab ? "rgba(139,92,246,0.25)" : "rgba(59,130,246,0.25)"}` }}>
                      {isLab ? "🔬 Lab" : "🎓 Lecture"}
                    </span>
                  </td>
                  <td><span style={{ fontFamily: "'DM Mono',monospace", fontSize: 12, color: "rgba(255,255,255,0.5)", fontWeight: 700 }}>{r.capacity}</span></td>
                  <td>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: isActive ? "rgba(34,197,94,0.1)" : "rgba(255,255,255,0.04)", color: isActive ? "#86efac" : "rgba(255,255,255,0.28)", border: `1px solid ${isActive ? "rgba(34,197,94,0.2)" : "rgba(255,255,255,0.07)"}` }}>
                      <span style={{ width: 5, height: 5, borderRadius: "50%", background: isActive ? "#22C55E" : "rgba(255,255,255,0.2)", display: "inline-block" }} />
                      {isActive ? "Active" : "Inactive"}
                    </span>
                  </td>
                  <td style={{ textAlign: "right" }}>
                    {isActive && <button className="tc-rm-btn-danger" onClick={() => handleDeactivate(r.id, r.name)}>Deactivate</button>}
                  </td>
                </tr>
              );
            })}
      ...
      <div style={{ marginTop: 12, fontSize: 11, color: "rgba(255,255,255,0.2)", fontFamily: "'DM Mono',monospace", letterSpacing: "0.06em" }}>
        {filtered.length} ROOM{filtered.length !== 1 ? "S" : ""}{search && ` · FILTERED FROM ${rooms.length}`}
      </div>
    </div>
  );
}