...
- **Never store plain passwords** — use `BCryptPasswordEncoder` from Spring Security
- **Short JWT expiry** — 15 min access token + 7 day refresh token
- **Method-level security** — use `@PreAuthorize("hasRole('ADMIN')")` on sensitive service methods
- **Never hardcode credentials** — use `application.yml` with environment variable references (`${DB_PASSWORD}`)
- **CORS config** — explicitly whitelist `http://localhost:3000` in dev, your production domain in prod

### 📐 Clean Architecture

- Strictly follow **Controller → Service → Repository** — never skip layers
- **Always use DTOs** — never expose JPA entity objects directly in API responses
- **One service per domain** — `ScheduleService` only does scheduling, not user management
- **Global exception handler** — use `@ControllerAdvice` to return consistent JSON errors to the frontend

### 📈 Scalability

- **Pagination on all list endpoints** — use `Pageable` from Spring Data JPA
- **Cache schedule reads** — use Spring Cache + Redis for timetable queries (read-heavy workload)
- **Async generation** — run schedule generation in a background thread with `@Async` for large datasets
- **Database indexes** — add indexes on `teacher_id`, `room_id`, `day_of_week`, `start_time` in the schedules table

### 🔮 Future Improvements

| Improvement | Technology |
|---|---|
| Advanced scheduling algorithm | OptaPlanner (Red Hat CSP solver) |
| Real-time schedule updates | WebSocket / Server-Sent Events |
| Email notifications on changes | Spring Mail + SMTP |
| PDF timetable export | iText or JasperReports |
| Containerized deployment | Docker + docker-compose |
| API documentation | Springdoc OpenAPI (Swagger UI) |

### 📋 Migration Best Practices

- ✅ One migration file per concern (one table per file)
- ✅ Always test migrations on a dev DB before running in production
- ✅ Commit migration files to Git — they are part of the codebase
- ❌ Never modify an already-applied migration file
- ❌ Never delete a migration file from the project

---

## 🔍 Find & Replace Reference

Replace all `YOUR_*` placeholders before running the project.
...
spring:
  datasource:
    url: jdbc:postgresql://YOUR_DB_HOST:YOUR_DB_PORT/YOUR_DB_NAME
    username: YOUR_DB_USER
    password: YOUR_DB_PASSWORD
    driver-class-name: org.postgresql.Driver
  jpa:
    hibernate:
      ddl-auto: validate          # Flyway manages the schema — never use create/update
    show-sql: false
  flyway:
    enabled: true
...
server:
  port: YOUR_BACKEND_PORT         # default: 8080

jwt:
  secret: YOUR_JWT_SECRET         # min 32 characters
  expiration: YOUR_JWT_EXPIRY_MS  # 900000 = 15 minutes

cors:
  allowed-origin: YOUR_CORS_ORIGIN  # http://localhost:3000
```
...
|---|---|---|
| `YOUR_DB_HOST` | PostgreSQL server hostname | `localhost` |
| `YOUR_DB_PORT` | PostgreSQL port | `5432` |
| `YOUR_DB_NAME` | Database name | `timecraft_db` |
| `YOUR_DB_USER` | Database username | `timecraft_user` |
| `YOUR_DB_PASSWORD` | Database password | `secure_password_here` |
| `YOUR_JWT_SECRET` | JWT signing key (min 32 chars) | `timecraft_super_secret_key_2025!` |
| `YOUR_JWT_EXPIRY_MS` | Access token expiry in ms | `900000` (15 min) |
| `YOUR_BACKEND_PORT` | Spring Boot server port | `8080` |
| `YOUR_FRONTEND_PORT` | React dev server port | `3000` |
| `YOUR_API_BASE_URL` | React `.env` Axios base URL | `http://localhost:8080/api` |
| `YOUR_CORS_ORIGIN` | Allowed CORS origin in Spring | `http://localhost:3000` |
| `YOUR_APP_PACKAGE` | Java base package name | `com.timecraft` |
...
```bash
git clone https://github.com/YOUR_USERNAME/timecraft.git
cd timecraft
```
...
```sql
CREATE DATABASE timecraft_db;
CREATE USER timecraft_user WITH PASSWORD 'YOUR_DB_PASSWORD';
GRANT ALL PRIVILEGES ON DATABASE timecraft_db TO timecraft_user;
```
...
# Edit application.yml — replace all YOUR_* placeholders
```
...
**TimeCraft** · Built with ☕ Java, ⚛ React, 🐘 PostgreSQL, and ✈ Flyway
