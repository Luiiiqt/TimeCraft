 ...
 *
 * Responsible for detecting and reporting all scheduling conflicts.
 * This service is called in two contexts:
 *
 * 1. Pre-assignment (by SchedulingEngine) — to validate before saving
 * 2. Post-assignment audit (by admin or scheduled job) — to scan existing
 * schedules and surface any conflicts introduced by manual overrides
 *
 * Each check returns a ConflictResult which contains the conflict type,
 * a human-readable description, and whether it is blocking (hard conflict)
 * or advisory (soft warning).
 *
 * Hard conflicts (block scheduling):
 ...
 * - WRONG_DEPARTMENT (teacher outside their dept — flagged, not blocked)
 */
@Slf4j
@Service
...
public class ConflictDetectionService {
    ...
    /**
     * Runs all hard conflict checks for a proposed schedule assignment.
     * Returns a list of detected conflicts. Empty list = no conflicts.
     *
    ...
    public List<ConflictResult> checkPreAssignment(
        ...
        conflicts.addAll(checkTeacherAvailability(teacherId, timeslot1Id));
        conflicts.addAll(checkTeacherAvailability(teacherId, timeslot2Id));
    ...
    /**
     * Checks if a teacher has declared themselves unavailable at this timeslot.
     */
    public List<ConflictResult> checkTeacherAvailability(Long teacherId,
            Long timeslotId) {
        boolean available = availabilityRepository
                .existsByTeacherIdAndTimeslotIdAndAvailableTrue(
                        teacherId, timeslotId);

        if (!available) {
            // No availability record or marked false = unavailable
            boolean hasRecord = availabilityRepository
                    .findByTeacherIdAndTimeslotId(teacherId, timeslotId)
                    .isPresent();

            String description = hasRecord
                    ? "Teacher has marked themselves unavailable at this timeslot"
                    : "Teacher has no availability declaration for this timeslot";
    ...
    /**
     * Checks if a teacher is already assigned to another class at this timeslot.
     * Applies to both session 1 and session 2 columns.
     * No year-level restriction — teacher conflicts are global.
     */
    public List<ConflictResult> checkTeacherDoubleBooked(Long teacherId,
        ...
        List<Schedule> existing = scheduleRepository.findTeacherConflicts(
                teacherId, timeslotId, semester, schoolYear);
    ...
    /**
     * Checks if a room is already booked at this timeslot.
     * Applies to both session 1 and session 2 columns.
     */
    public List<ConflictResult> checkRoomDoubleBooked(Long roomId,
        ...
        List<Schedule> existing = scheduleRepository.findRoomConflicts(
                roomId, timeslotId, semester, schoolYear);
    ...
    /**
     * Checks if a section already has a class at this timeslot.
     * Prevents two subjects in the same block section at the same time.
     */
    public List<ConflictResult> checkSectionConflict(Long sectionId,
        ...
        List<Schedule> existing = scheduleRepository.findSectionConflicts(
                sectionId, timeslotId, semester, schoolYear);
    ...
    /**
     * Checks if an individual student (irregular) has a time conflict
     ...
     * LABORATORY subject → must have LABORATORY room.
     * LECTURE subject → must have LECTURE room.
     */
    ...
    /**
     * Scans ALL published and draft schedules for a term and surfaces
     * any conflicts introduced by manual admin overrides.
     * Logs new conflict entries for any violations found.
     *
