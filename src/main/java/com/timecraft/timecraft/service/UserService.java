...
public class UserService {
    ...
    public List<User> findByUserTypeAndIsActiveTrue(UserType type) {
        return userRepository.findByUserTypeAndIsActiveTrue(type);
    }

    @Transactional
    public void updateUser(Long id, java.util.Map<String, Object> body) {
    ...
    // ── Program Head creation (Admin) ─────────────────────────────────────────
    @Transactional
    public User createProgramHead(String fullName, String email, String schoolId,
            Long departmentId, List<Long> courseIds, UserType userType,
            com.timecraft.timecraft.repository.DepartmentRepository departmentRepo,
            com.timecraft.timecraft.repository.DeanProfileRepository phProfileRepo,
            com.timecraft.timecraft.repository.DeanCourseRepository phCourseRepo,
            com.timecraft.timecraft.repository.CourseRepository courseRepo) {
        validateNewUser(email, schoolId);
        ...
        User user = userRepository.save(User.builder()
                ...
                .email(email)
                .schoolId(schoolId)
                .passwordHash(passwordEncoder.encode(schoolId))
                .userType(userType != null ? userType : UserType.DEAN)
                .isActive(true)
                .build());
        ...
        phProfileRepo.save(com.timecraft.timecraft.model.DeanProfile.builder()
                .user(user)
                .department(dept)
                .build());
        ...
        for (Long courseId : courseIds) {
            courseRepo.findById(courseId).ifPresent(course -> {
                phCourseRepo.save(
                        com.timecraft.timecraft.model.DeanCourse.builder()
                                .deanUserId(user.getId())
